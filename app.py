...
form{max-width:400px;width:100%;background:white;padding:40px;border-radius:20px;box-shadow:0 20px 40px rgba(0,0,0,0.2);}
input{width:100%;padding:15px;margin:10px 0;border:2px solid #ddd;border-radius:10px;font-size:16px;box-sizing:border-box;}
button{width:100%;padding:18px;background:#4ecdc4;color:white;border:none;border-radius:10px;font-size:18px;font-weight:bold;cursor:pointer;}
h1{color:white;text-align:center;margin-bottom:30px;}</style></head>
        ...
        if folders or items:
            ...
            for item in sorted(items):
                ...
                <div class="item-card">
                    ...
                    <div class="item-info">{item_data.get("info", "")}</div>
                    {photo_html}
                </div>
                '''
            content += '</div>'
...
.container{{max-width:1200px;margin:0 auto;background:white;border-radius:20px;padding:30px;box-shadow:0 15px 35px rgba(0,0,0,0.1);}}
.breadcrumbs{{margin:30px 0;padding:20px;background:#e9ecef;border-radius:15px;font-size:16px;}}
.breadcrumbs a{{color:#007bff;text-decoration:none;}}
.grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:25px;}}
.folder-card{{background:#e3f2fd;padding:25px;border-radius:20px;border-left:5px solid #2196f3;text-decoration:none;display:block;text-align:center;transition:all 0.3s;}}
...
.empty-folder{{text-align:center;color:#666;font-size:24px;margin:80px 0;padding:40px;background:#f8f9fa;border-radius:20px;border:2px dashed #ddd;}}
@media (max-width:768px){{.grid{{grid-template-columns:1fr;gap:15px;}}.container{{padding:20px;margin:10px;border-radius:15px;}}}}</style></head>
<body>
<div class="container">
...
def profiles():
    ...
    for username in sorted(users.keys()):
        profiles_html += f'''
        <div style="background:white;padding:25px;border-radius:20px;box-shadow:0 10px 30px rgba(0,0,0,0.1);text-align:center;">
            <h3 style="margin-bottom:15px;">{username}</h3>
            <a href="/profile/{username}" style="display:inline-block;padding:12px 24px;background:#007bff;color:white;border-radius:10px;font-weight:bold;">👁️ Профиль</a>
        </div>
        '''
...
.profiles-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:25px;}}
.back-btn{{background:#007bff;color:white;padding:20px 40px;border-radius:15px;font-size:18px;font-weight:bold;text-decoration:none;display:block;margin:50px auto;max-width:300px;text-align:center;}}
...
def profile(username):
    if username not in users:
        return '''
        <!DOCTYPE html>
        <html><head><title>Ошибка</title><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
        <body style="background:#f0f2f5;padding:100px;text-align:center;font-family:Arial;">
            <h1 style="color:#dc3545;">❌ Пользователь не найден</h1>
            <a href="/" style="background:#007bff;color:white;padding:15px 30px;border-radius:10px;font-size:18px;text-decoration:none;display:inline-block;margin-top:20px;">🏠 Главная</a>
        </body></html>
        '''
...
.profile-card{{background:white;max-width:800px;margin:auto;padding:40px;border-radius:25px;box-shadow:0 20px 60px rgba(0,0,0,0.1);text-align:center;}}
.role-badge{{padding:15px 30px;background:{role_color};color:white;border-radius:25px;font-size:20px;font-weight:bold;display:inline-block;margin:20px 0;}}
.status{{font-size:18px;color:#666;margin:30px 0;padding:20px;background:#e8f5e8;border-radius:15px;}}
.back-btn{{background:#007bff;color:white;padding:18px 40px;border-radius:15px;font-size:18px;font-weight:bold;display:inline-block;margin-top:40px;}}</style></head>
<body>
<div class="profile-card">
    <h1 style="margin-bottom:20px;">{username}</h1>
    <div class="role-badge">{role_display}</div>
    <div class="status">{profile_data.get("status", "Онлайн")}</div>
    <a href="/" class="back-btn">🏠 Главная</a>
</div>
</body></html>
    '''
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>body{{font-family:Arial,sans-serif;padding:30px;background:#f8f9fa;}}
.container{{max-width:600px;margin:auto;background:white;border-radius:20px;padding:30px;box-shadow:0 15px 40px rgba(0,0,0,0.1);}}
.section{{background:#f8f9fa;margin:25px 0;padding:30px;border-radius:16px;box-shadow:0 5px 20px rgba(0,0,0,0.08);}}
input,textarea{{width:100%;padding:15px;margin:10px 0;border:2px solid #ddd;border-radius:10px;font-size:16px;box-sizing:border-box;}}
button{{width:100%;padding:18px;background:#28a745;color:white;border:none;border-radius:12px;font-size:16px;font-weight:bold;cursor:pointer;margin:10px 0;}}
h1{{text-align:center;color:#333;margin-bottom:30px;}}
h2{{color:#333;margin-bottom:20px;border-left:5px solid #28a745;padding-left:15px;}}</style></head>
<body>
<div class="container">
    ...
    <div style="text-align:center;margin-top:40px;">
        <a href="/" style="background:#007bff;color:white;padding:20px 40px;border-radius:15px;font-size:18px;font-weight:bold;text-decoration:none;display:inline-block;">🏠 Главная</a>
    </div>
</div>
</body></html>
    '''
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>body{font-family:Arial,sans-serif;padding:100px 20px;text-align:center;background:#f8f9fa;}
h1{font-size:48px;margin-bottom:30px;}
a[href="https://t.me/ssylkanatelegramkanalyznaikin"]{{font-size:28px;color:#0088cc;text-decoration:none;font-weight:bold;}}
.back-btn{{background:#007bff;color:white;padding:20px 40px;border-radius:15px;font-size:18px;font-weight:bold;text-decoration:none;display:inline-block;margin-top:50px;}}</style></head>
<body>
<h1>💬 Сообщество</h1>
<p><a href="https://t.me/ssylkanatelegramkanalyznaikin">Telegram канал</a></p>
<a href="/" class="back-btn">🏠 Главная</a>
</body></html>
    '''
