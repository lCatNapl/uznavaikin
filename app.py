...
# Остальные роуты остаются такими же (profiles, profile, admin, community, logout)
@app.route('/profiles')
def profiles():
...
def admin():
    ...
    return f'''<!DOCTYPE html><html><head><title>Админка</title><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{{font-family:Arial,sans-serif;padding:20px;background:#f8f9fa;}}.container{{max-width:800px;margin:auto;background:white;border-radius:20px;padding:30px;box-shadow:0 15px 40px rgba(0,0,0,0.1);}}.section{{background:#f8f9fa;margin:25px 0;padding:30px;border-radius:16px;box-shadow:0 5px 20px rgba(0,0,0,0.08);}}h2{{color:#333;margin-bottom:20px;border-left:5px solid #28a745;padding-left:15px;}}input{{width:100%;padding:15px;margin:10px 0;border:2px solid #ddd;border-radius:10px;font-size:16px;box-sizing:border-box;}}button{{width:100%;padding:18px;background:#28a745;color:white;border:none;border-radius:12px;font-size:16px;font-weight:bold;cursor:pointer;margin:10px 0;}}.back-btn{{background:#007bff;display:block;margin:40px auto;padding:20px 40px;max-width:300px;border-radius:15px;font-size:18px;font-weight:bold;text-decoration:none;text-align:center;}}</style></head><body><div class="container"><h1 style="text-align:center;color:#333;margin-bottom:30px;">🔧 АДМИН ПАНЕЛЬ</h1><div class="section"><h2>🔇 МУТИТЬ ИГРОКА</h2><form method="post"><input type="hidden" name="action" value="mute"><input name="target" placeholder="Имя игрока" required><input name="duration" type="number" placeholder="Минуты" value="5" min="1"><input name="reason" placeholder="Причина" required><button>🔇 ЗАМУТИТЬ</button></form></div>{'''<div class="section"><h2>👮 ДАТЬ МОДЕРАТОРА</h2><form method="post"><input type="hidden" name="action" value="add_mod"><input name="target" placeholder="Имя игрока" required><input name="duration" type="number" placeholder="Часы" value="1" min="1"><button>👮 НАЗНАЧИТЬ</button></form></div>''' if can_add_mod else ''}<a href="/" class="back-btn">🏠 Главная</a></div></body></html>'''
