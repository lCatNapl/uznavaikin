...
app.secret_key = 'uznavaykin-v30-secret-2026'
...
user_activity = {k: float(v) for k, v in data.get('user_activity', {}).items()}
chat_messages = data.get('chat_messages', [])
moderators = {k: float(v) for k, v in data.get('moderators', {}).items()}
mutes = {k: float(v) for k, v in data.get('mutes', {}).items()}

# 🔥 КАТАЛОГ
catalog = data.get('catalog', {
    'Каталог': {
        'Minecraft': {
            'Алмаз': {'location': 'Каталог/Minecraft', 'info': 'Самый ценный ресурс!', 'photo': ''},
            'Железо': {'location': 'Каталог/Minecraft', 'info': 'Для инструментов', 'photo': ''}
        },
        'World of Tanks': {
            'Т-34': {'location': 'Каталог/World of Tanks', 'info': 'Легендарный танк СССР', 'photo': ''},
            'IS-7': {'location': 'Каталог/World of Tanks', 'info': 'Тяжелый танк 10 уровня', 'photo': ''}
        }
    }
})

designs = data.get('designs', {
    'start': 'basic', 'vip': 'vip', 'premium': 'premium', 'moderator': 'admin', 'admin': 'admin'
})
...
def index():
    ...
    html = f'''<!DOCTYPE html>
<html><head><title>🚀 Узнавайкин v30</title>
<meta charset="utf-8">
...
<body><div class="container">'''
    ...
    if current_user:
        html += f'<div class="header"><h1>🚀 Узнавайкин v30</h1><p>👤 <b style="font-size:1.2em;">{current_user}</b> | <span style="background:rgba(255,255,255,0.3);padding:8px 15px;border-radius:15px;font-size:1.1em;font-weight:bold;">{get_role_display(current_user)}</span></p></div>'
    else:
        html += '<div class="header"><h1>🚀 Узнавайкин v30</h1><p style="font-size:1.2em;">Добро пожаловать в лучший чат!</p></div>'
...
<a href="/" style="background:#28a745;color:white;padding:15px 30px;border-radius:10px;text-decoration:none;display:inline-block;margin-left:10px;font-size:18px;">🏠 Главная</a>
    ...
    for folder in sorted(content['folders']):
        content_html += f'<a href="/catalog/{path}/{folder}" style="background:#e3f2fd;padding:30px;border-radius:20px;border-left:5px solid #2196f3;text-decoration:none;display:block;text-align:center;transition:all 0.3s;font-family:Arial;"><h3 style="margin:0 0 10px 0;color:#2196f3;font-size:1.5em;">📁 {folder}</h3><p style="margin:0;color:#666;font-size:1.1em;">Папка</p></a>'
    
    for item_name, item_data in sorted(content['items']):
        content_html += f'''
        <div style="background:#f3e5f5;padding:30px;border-radius:20px;border-left:5px solid #9c27b0;font-family:Arial;">
            <h3 style="font-size:1.8em;font-weight:bold;margin-bottom:15px;color:#333;">{item_name}</h3>
            <p style="margin:5px 0;"><b style="color:#555;">📍 Местоположение:</b> {item_data.get("location", "Каталог")}</p>
            <p style="margin:5px 0;line-height:1.6;"><b style="color:#555;">ℹ️ Информация:</b> {item_data.get("info", "—")}</p>
        </div>'''
    
    content_html += '</div>'
    
    if not content['folders'] and not content['items']:
        content_html = '<div style="text-align:center;color:#666;font-size:2em;margin:80px 0;padding:60px;background:#f8f9fa;border-radius:25px;border:3px dashed #ddd;font-family:Arial;">📭 Эта папка пуста</div>'
    
    return f'''<!DOCTYPE html>
<html><head><title>📁 Каталог {path or ""}</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>body{{font-family:Arial,sans-serif;padding:20px;background:#f8f9fa;}}.container{{max-width:1200px;margin:0 auto;background:white;border-radius:25px;padding:40px;box-shadow:0 20px 60px rgba(0,0,0,0.1);}}.breadcrumbs{{margin:30px 0;padding:25px;background:#e9ecef;border-radius:20px;font-size:18px;}}.back-btn{{background:#007bff;color:white;padding:18px 40px;border-radius:15px;font-size:20px;font-weight:bold;text-decoration:none;display:inline-block;margin:40px 10px;}}@media (max-width:768px){{.container{{padding:20px;margin:10px;border-radius:20px;}}.grid{{grid-template-columns:1fr !important;gap:20px;}}}}</style></head>
<body><div class="container">
...
<div class="login-container">
    ...
    <p style="margin-top:30px;font-size:14px;">Пароли не хранятся в открытом виде</p>
</div></body></html>'''
...
def admin():
    ...
    return f'''<!DOCTYPE html>
<html><head><title>🔧 Админ-панель v30</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>body{{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#ff6b6b,#4ecdc4);padding:20px;min-height:100vh;}}.container{{max-width:1000px;margin:auto;background:white;border-radius:25px;padding:40px;box-shadow:0 25px 80px rgba(0,0,0,0.2);}}.section{{background:#f8f9fa;margin:25px 0;padding:30px;border-radius:20px;box-shadow:0 10px 30px rgba(0,0,0,0.1);}}h2{{color:#333;margin-bottom:20px;border-left:5px solid #00b894;padding-left:15px;font-size:1.8em;}}input{{width:100%;padding:15px;margin:10px 0;border:2px solid #ddd;border-radius:12px;font-size:16px;box-sizing:border-box;}}button{{width:100%;padding:18px;margin:10px 0;border:none;border-radius:12px;font-size:16px;font-weight:bold;cursor:pointer;transition:all 0.3s;}}button:hover{{transform:translateY(-2px);box-shadow:0 10px 25px rgba(0,0,0,0.2);}}.btn-mute{{background:#ff6b8a;color:white;}}.btn-mod{{background:#00b894;color:white;}}.btn-add{{background:#00cec9;color:white;}}.btn-delete{{background:#e17055;color:white;}}.message{{padding:20px;margin:20px 0;border-radius:15px;font-weight:bold;text-align:center;font-size:18px;}}.catalog-list{{background:#e3f2fd;padding:20px;border-radius:15px;max-height:250px;overflow:auto;margin:15px 0;}}</style></head>
<body><div class="container">
<h1 style="text-align:center;color:#333;margin-bottom:40px;font-size:2.5em;">🔧 Админ-панель v30</h1>
...
<input name="reason" placeholder="Причина (до 100 символов)" maxlength="100" required>
...
<button class="btn-mod">👮 НАЗНАЧИТЬ</button></form>
...
<button class="btn-delete">❌ СНЯТЬ</button></form></div>

<div class="section"><h2>➕ КАТАЛОГ</h2>
<form method="post"><input type="hidden" name="action" value="add_item">
<input name="path" placeholder="Путь (Minecraft)" required maxlength="100">
<input name="name" placeholder="Название (Алмаз)" required maxlength="50">
<input name="info" placeholder="Описание" required maxlength="200">
<button class="btn-add">➕ ДОБАВИТЬ ПРЕДМЕТ</button></form>

<form method="post"><input type="hidden" name="action" value="add_folder">
<input name="path" placeholder="Путь (Каталог)" required maxlength="100">
<input name="name" placeholder="Название (CS2)" required maxlength="50">
...
<input name="path" placeholder="Полный путь (Каталог/Minecraft/Алмаз)" required maxlength="150">
<button class="btn-delete">🗑️ УДАЛИТЬ</button></form>

<div class="catalog-list">
<h3 style="margin-top:0;">📋 Текущий каталог:</h3>
<pre style="margin:0;font-size:14px;">{json.dumps(catalog.get("Каталог", {}), ensure_ascii=False, indent=2)[:800]}...</pre>
</div></div>

<a href="/" style="display:block;text-align:center;background:#007bff;color:white;padding:25px 50px;border-radius:20px;font-size:22px;font-weight:bold;text-decoration:none;margin:50px auto 20px;max-width:400px;">🏠 На главную</a>
</div></body></html>'''
...
def profiles():
    ...
    for user in sorted(users.keys()):
        ...
        <div style="background:white;padding:30px;border-radius:20px;box-shadow:0 15px 40px rgba(0,0,0,0.1);text-align:center;margin:20px;">
            ...
            <div style="padding:15px 25px;background:#e8f5e8;border-radius:15px;font-size:1.3em;font-weight:bold;margin:20px 0;">{get_role_display(user)}</div>
            <p style="color:#666;margin:10px 0;">{profile.get("status", "Онлайн")}</p>
            <a href="/profile/{user}" style="display:inline-block;padding:15px 35px;background:#007bff;color:white;border-radius:15px;font-weight:bold;font-size:18px;text-decoration:none;">👁️ Профиль</a>
        </div>'''
...
def profile(username):
    if username not in users:
        return '<h1 style="color:red;text-align:center;padding:100px;font-size:3em;">❌ Пользователь не найден</h1>'
    ...
    <form method="post" style="margin-top:20px;">
        <input name="status" value="{profile_data.get("status", "Онлайн")}" placeholder="Статус" maxlength="50" style="width:100%;padding:15px;border:2px solid #ddd;border-radius:12px;font-size:16px;">
    </form>''' if is_owner else f'<div style="font-size:1.3em;color:#27ae60;padding:20px;background:#e8f5e8;border-radius:15px;">{profile_data.get("status", "Онлайн")}</div>'
    
    info_html = f'''
    <form method="post" style="margin-top:20px;">
        <textarea name="info" placeholder="Расскажите о себе..." rows="6" maxlength="500" style="width:100%;padding:15px;border:2px solid #ddd;border-radius:12px;font-size:16px;font-family:Arial;">{profile_data.get("info", "")}</textarea>
        <button type="submit" style="width:100%;padding:15px;background:#3498db;color:white;border:none;border-radius:12px;font-size:16px;margin-top:15px;cursor:pointer;">💾 Сохранить</button>
    </form>''' if is_owner else f'<div style="padding:25px;background:#f8f9fa;border-radius:15px;line-height:1.7;font-size:1.1em;border-left:5px solid #3498db;">{profile_data.get("info", "Информация не указана")}</div>'
    
    return f'''<!DOCTYPE html><html><head><title>👤 {username} - Профиль</title><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{{font-family:Arial,sans-serif;padding:40px;background:#f0f2f5;}}.profile-card{{background:white;max-width:800px;margin:auto;padding:50px;border-radius:30px;box-shadow:0 25px 80px rgba(0,0,0,0.15);text-align:center;}}.role-badge{{padding:20px 40px;background:{role_color};color:white;border-radius:30px;font-size:1.8em;font-weight:bold;display:inline-block;margin:30px 0;box-shadow:0 10px 30px rgba(0,0,0,0.2);}}.back-btn{{background:#007bff;color:white;padding:20px 50px;border-radius:20px;font-size:20px;font-weight:bold;display:inline-block;margin-top:40px;text-decoration:none;}}@media (max-width:768px){{.profile-card{{padding:30px;margin:20px;border-radius:25px;}}}}</style></head><body><div class="profile-card"><h1 style="font-size:3em;margin-bottom:20px;color:#333;">{username}</h1><div class="role-badge">{role_display}</div>{status_html}<div style="margin:40px 0;padding:30px;background:#ecf0f1;border-radius:20px;"><h2 style="color:#2c3e50;margin-bottom:20px;">О себе:</h2>{info_html}</div><a href="/" class="back-btn">🏠 Главная</a>{" | <a href='/profiles' style='background:#28a745;color:white;padding:20px 50px;border-radius:20px;font-size:20px;font-weight:bold;display:inline-block;margin-left:20px;text-decoration:none;'>👥 Профили</a>" if current_user else ""}</div></body></html>'''

@app.route('/community')
def community():
    return '''<!DOCTYPE html><html><head><title>💬 Сообщество</title><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{font-family:'Segoe UI',Arial,sans-serif;padding:100px 20px;text-align:center;background:linear-gradient(135deg,#667eea,#764ba2);color:white;min-height:100vh;}.community-box{max-width:600px;margin:auto;background:rgba(255,255,255,0.1);backdrop-filter:blur(20px);padding:80px;border-radius:30px;box-shadow:0 30px 100px rgba(0,0,0,0.3);}.tg-link{font-size:3em;color:#0088cc;text-decoration:none;font-weight:bold;display:inline-block;margin:40px 0;padding:25px 50px;background:rgba(255,255,255,0.2);border-radius:25px;transition:all 0.3s;}.tg-link:hover{transform:scale(1.05);background:rgba(255,255,255,0.3);}.back-btn{background:#007bff;color:white;padding:25px 60px;border-radius:25px;font-size:24px;font-weight:bold;text-decoration:none;display:inline-block;margin-top:60px;box-shadow:0 15px 40px rgba(0,0,0,0.3);transition:all 0.3s;}.back-btn:hover{transform:translateY(-3px);box-shadow:0 20px 50px rgba(0,0,0,0.4);}}</style></head><body><div class="community-box"><h1 style="font-size:4em;margin-bottom:40px;">💬 Сообщество</h1><p style="font-size:1.5em;margin-bottom:50px;">Присоединяйся к нам!</p><a href="https://t.me/ssylkanatelegramkanalyznaikin" class="tg-link" target="_blank">📱 Telegram</a><a href="/" class="back-btn">🏠 Главная</a></div></body></html>'''
